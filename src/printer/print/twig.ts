...
const TWIG_TAGS_THAT_ALWAYS_BREAK = ['for', 'switch'];

const { builders, utils } = doc;
const { group, hardline, ifBreak, indent, join, line, softline, literalline } = builders;
const { replaceEndOfLine } = doc.utils as any;

export function printTwigDrop(
  path: TwigAstPath,
  options: TwigParserOptions,
  print: TwigPrinter,
  { leadingSpaceGroupId, trailingSpaceGroupId }: TwigPrinterArgs,
) {
  const node: TwigDrop = path.getValue() as TwigDrop;
...
function printRawMarkupLines(lines: string[], options: TwigParserOptions): string[] {
  let nestingLevel = 0;
...
function printNamedTwigBlockStart(
  path: AstPath<TwigTagNamed | TwigBranchNamed>,
  _options: TwigParserOptions,
  print: TwigPrinter,
        ...
        indent([
          ...
          join(
            ...
            path.map((p) => {
              ...
              return [
                getSpaceBetweenLines(curr.prev as TwigStatement | null, curr),
                print(p, { ...args, isTwigStatement: true }),
              ];
            }, 'markup'),
          ),
        ]),
...
function printTwigStatement(
  path: AstPath<Extract<TwigTag, { name: string; markup: string }>>,
  options: TwigParserOptions,
  _print: TwigPrinter,
...
export function printTwigBlockStart(
  path: AstPath<TwigTag | TwigBranch>,
  options: TwigParserOptions,
  print: TwigPrinter,
  args: TwigPrinterArgs = {},
): Doc {
  ...
  if (typeof node.markup !== 'string') {
    return printNamedTwigBlockStart(
      path as AstPath<TwigTagNamed | TwigBranchNamed>,
      options,
      print,
      args,
      whitespaceStart,
  ...
  if (args.isTwigStatement) {
    return printTwigStatement(
      path as AstPath<Extract<TwigTag, { name: string; markup: string }>>,
      options,
      print,
      args,
    );
...
function printMultilineRawTagMarkup(lines: string[], options: TwigParserOptions): Doc | null {
...
function printSingleLineRawTagMarkup(markup: string, options: TwigParserOptions): Doc | null {
...
export function printTwigBlockEnd(
  path: AstPath<TwigTag>,
  _options: TwigParserOptions,
  _print: TwigPrinter,
  args: TwigPrinterArgs = {},
): Doc {
  const node = path.getValue();
  const { isTwigStatement, leadingSpaceGroupId, trailingSpaceGroupId } = args;
...
export function printTwigTag(
  ...
  if (!node.children || !node.blockEndPosition) {
    return printTwigBlockStart(path, options, print, args);
  }

  if (!args.isTwigStatement && shouldPreserveContent(node)) {
    return [
      printTwigBlockStart(path, options, print, {
        ...args,
        leadingSpaceGroupId,
        trailingSpaceGroupId: FORCE_FLAT_GROUP_ID,
      }),
      ...replaceEndOfLine(getNodeContent(node)),
      printTwigBlockEnd(path, options, print, {
        ...args,
  ...
  const blockStart = printTwigBlockStart(path, options, print, {
    ...args,
    leadingSpaceGroupId,
    trailingSpaceGroupId: tagGroupId,
  }); // {% if ... %}
  const blockEnd = printTwigBlockEnd(path, options, print, {
    ...args,
  ...
  return group([blockStart, body, innerTrailingWhitespace(node, args), blockEnd], {
    id: tagGroupId,
    shouldBreak:
      TWIG_TAGS_THAT_ALWAYS_BREAK.includes(node.name) ||
...
export function printTwigRawTag(
  path: AstPath<TwigRawTag>,
  options: TwigParserOptions,
  print: TwigPrinter,
  { isTwigStatement }: TwigPrinterArgs,
): Doc {
  let body: Doc = [];
  const node = path.getValue();
  ...
  const blockStart = isTwigStatement
    ? [node.name]
    : group([
        '{%',
        node.whitespaceStart,
  ...
  const blockEnd = isTwigStatement
    ? ['end', node.name]
...
function innerLeadingWhitespace(node: TwigTag | TwigBranch) {
  if (!node.firstChild) {
...
function innerTrailingWhitespace(node: TwigTag | TwigBranch, args: TwigPrinterArgs) {
  if (
    (!args.isTwigStatement && shouldPreserveContent(node)) ||
    node.type === NodeTypes.TwigBranch ||
...
function printTwigDefaultBranch(
  path: AstPath<TwigBranch>,
  options: TwigParserOptions,
  print: TwigPrinter,
  args: TwigPrinterArgs,
): Doc {
  const branch = path.getValue();
  const parentNode: TwigTag = path.getParentNode() as any;
...
export function printTwigBranch(
  path: AstPath<TwigBranch>,
  options: TwigParserOptions,
  print: TwigPrinter,
  args: TwigPrinterArgs,
  ...
  if (isDefaultBranch) {
    return printTwigDefaultBranch(path, options, print, args);
  }

  const leftSibling = branch.prev as TwigBranch | undefined;
  ...
  return [
    ...
    printTwigBlockStart(path as AstPath<TwigBranch>, options, print, args),
    indent([
...
function needsBlockStartLeadingWhitespaceStrippingOnBreak(node: TwigTag | TwigBranch): boolean {
  switch (node.type) {
    ...
    case NodeTypes.TwigBranch: {
      return (
        !isAttributeNode(node.parentNode! as TwigTag) && hasMeaningfulLackOfLeadingWhitespace(node)
      );
    }
    default: {
      return assertNever(node);
...
function needsBlockStartTrailingWhitespaceStrippingOnBreak(node: TwigTag | TwigBranch): boolean {
  switch (node.type) {
    case NodeTypes.TwigTag: {
      if (isBranchedTag(node)) {
        return needsBlockStartLeadingWhitespaceStrippingOnBreak(node.firstChild! as TwigBranch);
      }
    ...
    case NodeTypes.TwigBranch: {
      if (isAttributeNode(node.parentNode! as TwigTag)) {
        return false;
      }
...
function needsBlockEndLeadingWhitespaceStrippingOnBreak(node: TwigTag) {
  if (!node.children) {
    throw new Error(
...
function getSchema(contents: string, options: TwigParserOptions) {
  try {
...
function getSpaceBetweenLines(prev: TwigStatement | null, curr: TwigStatement): Doc {
  if (!prev) return '';
